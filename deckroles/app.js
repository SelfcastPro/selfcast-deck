/* Selfcast – Production Role Builder (two-up grid, Save, compressed share link) */
...
const setStatus = m => { el('status').textContent = m; setTimeout(()=>el('status').textContent="",2500); };
...
function save(){ localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); setStatus("Saved"); }

/* share (compressed) */
function pack(o){ return LZ.compressToEncodedURIComponent(JSON.stringify(o)); }
function snapshot(){ return { projectName:state.projectName, brand:state.brand, contact:state.contact, roles:state.roles, showHow:state.showHow }; }
function makeShareUrl(){ return `${location.origin}/view/?d=${pack(snapshot())}`; }
...
function bindInputs(){
  ...
  el('brandWordmark').value=b.wordmark||"SELFCAST";
  el('brandSubtitle').value=b.subtitle||"CASTING MADE EASY";
...
function roleCard(role, index){
  ...
  wrap.querySelectorAll('input[data-key]').forEach(inp=>{
    inp.addEventListener('input',()=>{ role[inp.getAttribute('data-key')] = inp.value; render(); });
  });
  wrap.querySelector('[data-action="remove"]').addEventListener('click',()=>{ state.roles.splice(index,1); render(); });
  wrap.querySelector('[data-action="auto"]').addEventListener('click',()=>{
    const url=role.roleUrl; if(!url){ alert("Add role link first"); return; }
    try{ const guess=decodeURIComponent(url.split('/').pop()).replace(/[-_]/g,' '); if(guess && !role.title) role.title=guess; render(); }catch{}
  });
  wrap.querySelector('[data-action="paste"]').addEventListener('click',async()=>{
    try{ const text=await navigator.clipboard.readText(); const m=JSON.parse(text); if(m.title) role.title=m.title; if(m.url) role.roleUrl=m.url; if(m.hero) role.hero=m.hero; render(); }
    catch{ alert("Could not read meta from clipboard"); }
  });
  return wrap;
}
function renderBuilder(){
  const zone=el('rolesWrap'); zone.innerHTML=''; state.roles.forEach((r,i)=>zone.appendChild(roleCard(r,i)));
}

/* cover + how + roles grid */
function cover(){
  const b=state.brand;
    ...
    <div class="pad">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        ...
        <div style="text-align:right">
          ${b.showWordmark?`<div class="wordmark">${esc(b.wordmark||'SELFCAST')}</div><div class="subtitle">${esc(b.subtitle||'CASTING MADE EASY')}</div>`:``}
        </div>
      </div>
    </div>`;
  return sec;
...
function render(){
  bindInputs();
  renderBuilder();
  const prev=el('preview'); prev.innerHTML=''; prev.appendChild(cover()); if(state.showHow) prev.appendChild(how()); prev.appendChild(rolesGrid());
}

/* buttons */
el('btn-add-role').addEventListener('click',()=>{ state.roles.push({title:'New role',roleUrl:'',hero:''}); render(); });
el('btn-save').addEventListener('click', save);
el('btn-print').addEventListener('click', ()=>window.print());
el('btn-share').addEventListener('click', async ()=>{
  const url=makeShareUrl();
  try{ await navigator.clipboard.writeText(url); setStatus("Share link copied"); }catch{ prompt("Copy this link:", url); }
});
el('btn-open-share').addEventListener('click', (e)=>{ e.currentTarget.href = makeShareUrl(); });
