(() => {
  ...
  function roleCard(role, index){
    ...
    role.talents.forEach((t,i)=>{
      ...
      row.querySelector('[data-action="del"]').addEventListener('click', ()=>{
        role.talents.splice(i,1); render();
      });
      ...
      <div class="pad">
        <div class="between" style="align-items:flex-start;">
          <div class="logo" style="color:#111">
            ${b.logoUrl ? `<img src="${escapeHtml(b.logoUrl)}" alt="Logo" style="height:40px;border-radius:8px;border:1px solid #eee"/>` : `<div class="mark" style="background:#e51c23;color:#fff">S</div>`}
      ...
      <div class="pad">
        <div class="between">
          <div>
            ...
            ${role.roleUrl?`<a class="small-link" href="${escapeHtml(role.roleUrl)}" target="_blank">Open role</a>`:''}
          </div>
              ...
              <div class="tbody">
                <div class="between">
                  <div>
                    ...
                    ${t.notes?`<div class="small">${escapeHtml(t.notes)}</div>`:''}
                  </div>
                  <span class="badge">${(STAGES.find(s=>s.key===t.stage)||{}).label||'Option'}</span>
                </div>
                ${t.profileUrl?`<a class="small-link" href="${escapeHtml(t.profileUrl)}" target="_blank">Open profile</a>`:''}
              </div>
            </div>
  ...
  function howItWorks(){
    ...
    sec.innerHTML = `
      <div class="pad">
        ...
        <p><strong>When you click Requested videos/photos</strong><br/>You can view videos and images uploaded for this specific job.</p>
        <p><strong>When you click on the Talent's picture</strong><br/>You can see the Talent's profile and information.</p>
        <p><strong>Add to Shortlist</strong><br/>Shortlist the Talents you want to pass on in the process...</p>
  ...
  // Global buttons
  safeBind(id('btn-add-role'),'click', ()=>{
    state.roles.push({ title:'New role', roleUrl:'', hero:'', filter:'all', talents:[] });
    render();
  });
  safeBind(id('btn-print'),'click', ()=> window.print());
